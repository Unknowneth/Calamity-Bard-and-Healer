...
{
	public class HydrogenSulfide : ScythePro
	{
		public override string Texture => "CalamityBardHealer/Items/HydrogenSulfide";
		...
		public override bool PreAI() {
			...
			if(base.Projectile.localAI[2] == 0f && base.Projectile.ai[1] < base.Projectile.ai[0] * 0.5f) {
				if(Main.myPlayer == player.whoAmI) for(int i = -1; i <= 1; i++) {
					int p = Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), base.Projectile.Center + Vector2.Normalize(base.Projectile.velocity).RotatedBy(MathHelper.PiOver4 * i) * base.Projectile.height * 0.6f, Vector2.Normalize(base.Projectile.velocity).RotatedBy(MathHelper.PiOver4 * i * 0.5f) * 8f, i == 0 ? ModContent.ProjectileType<Projectiles.HydrogenSulfideWave>() : ModContent.ProjectileType<Projectiles.HydrogenSulfideFlames>(), base.Projectile.damage, base.Projectile.knockBack, player.whoAmI);
		...
		public override void SafeOnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			for(int i = 0; i < 7; i++) {
				...
				GeneralParticleHandler.SpawnParticle(new MediumMistParticle(target.Center + offset, velOffset * Main.rand.NextFloat(1.5f, 3f), Color.OrangeRed, Color.Blue, Main.rand.NextFloat(0.9f, 1.2f), 160f, Main.rand.NextFloat(0.03f, -0.03f)));
			}
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			Main.EntitySpriteDraw(texture, base.Projectile.Center - Main.screenPosition, null, lightColor, base.Projectile.rotation + MathHelper.PiOver2 - MathHelper.ToRadians(base.Projectile.spriteDirection * 15f), new Vector2(texture.Width / 2 - base.Projectile.spriteDirection * 14, texture.Height - 18), base.Projectile.scale, base.Projectile.spriteDirection < 0 ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0);
			if(base.Projectile.ai[1] / base.Projectile.ai[0] < 0.75f) {
				Player player = Main.player[base.Projectile.owner];
				float swingTime = base.Projectile.ai[1] / (base.Projectile.ai[0] * 0.75f);
				for(int e = 0; e < 4; e++) swingTime = MathHelper.SmoothStep(0f, 1f, swingTime);
				lightColor = Color.Lerp(Color.OrangeRed, Color.Blue, swingTime * swingTime);
				lightColor.A = 0;
				...
				Main.EntitySpriteDraw(texture, player.MountedCenter - new Vector2(4, 2) * player.Directions - Main.screenPosition, null, lightColor * swingTime * 0.8f, base.Projectile.rotation + MathHelper.ToRadians(base.Projectile.spriteDirection * 15f), texture.Size() / 2, base.Projectile.scale * 1.9f, base.Projectile.spriteDirection < 0 ? SpriteEffects.FlipVertically : SpriteEffects.None, 0);
			}
			return false;
		}
