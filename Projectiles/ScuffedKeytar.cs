...
{
	public class ScuffedKeytar : BardProjectile
	{
		public override string GlowTexture => "Terraria/Images/Extra_98";
		public override void SetStaticDefaults() {
			ProjectileID.Sets.TrailCacheLength[base.Projectile.type] = 14;
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			if(base.Projectile.velocity != Vector2.Zero) for(int h = -2; h <= 2; h++) for(int i = 0; i < base.Projectile.oldPos.Length; i++) {
				lightColor = Color.Lerp(Color.Violet, Color.Purple, (float)i / (float)base.Projectile.oldPos.Length);
				lightColor.A = 0;
				lightColor *= fade;
