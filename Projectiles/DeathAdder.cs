...
{
	public class DeathAdder : ScythePro
	{
		public override string Texture => "CalamityBardHealer/Items/DeathAdder";
		...
		public override bool PreAI() {
			...
			if(base.Projectile.localAI[2] == 0f && base.Projectile.ai[1] <= base.Projectile.ai[0] * 0.5f) {
				if(Main.myPlayer == player.whoAmI) for(int i = -3; i <= 3; i++) {
					int p = Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), base.Projectile.Center + Vector2.Normalize(base.Projectile.velocity).RotatedBy(MathHelper.PiOver4 * 0.125f * i) * base.Projectile.height * 0.6f + Main.rand.NextVector2CircularEdge(i, i) * 2f, Vector2.Normalize(base.Projectile.velocity).RotatedBy(Main.rand.NextFloat(MathHelper.PiOver4 * 0.5f) - MathHelper.PiOver4 * 0.25f + MathHelper.PiOver4 * 0.25f * i) * Main.rand.NextFloat(4f, 8f), ModContent.ProjectileType<Projectiles.DeathAdderSoul>(), base.Projectile.damage, base.Projectile.knockBack, player.whoAmI);
		...
		public override void SafeOnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			...
			if(ModLoader.TryGetMod("CalamityMod", out Mod calamity)) target.AddBuff(calamity.Find<ModBuff>("WhisperingDeath").Type, 240);
		}
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			if(base.Projectile.ai[1] / base.Projectile.ai[0] < 0.75f) {
				...
				Main.EntitySpriteDraw(texture, player.MountedCenter - new Vector2(4, 2) * player.Directions - Main.screenPosition, null, lightColor * swingTime * 0.9f, base.Projectile.rotation + MathHelper.ToRadians(base.Projectile.spriteDirection * 15f), texture.Size() / 2, base.Projectile.scale * 2.3f, base.Projectile.spriteDirection < 0 ? SpriteEffects.FlipVertically : SpriteEffects.None, 0);
				lightColor = Color.Lerp(Color.Red, Color.Pink, 0.25f);
				lightColor.A = 0;
				texture = (Texture2D)ModContent.Request<Texture2D>("CalamityBardHealer/Projectiles/Slash_2");
				Main.EntitySpriteDraw(texture, player.MountedCenter - new Vector2(4, 2) * player.Directions - Main.screenPosition, null, lightColor * swingTime * 0.9f, base.Projectile.rotation + MathHelper.ToRadians(base.Projectile.spriteDirection * 15f), texture.Size() / 2, base.Projectile.scale * 2.6f, base.Projectile.spriteDirection < 0 ? SpriteEffects.FlipVertically : SpriteEffects.None, 0);
