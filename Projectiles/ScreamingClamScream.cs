...
{
	public class ScreamingClamScream : ModProjectile
	{
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			if(ModLoader.TryGetMod("ThoriumMod", out Mod thorium)) target.AddBuff(thorium.Find<ModBuff>("Tuned").Type, 120);
			if(ModLoader.TryGetMod("CalamityMod", out Mod calamity)) target.AddBuff(calamity.Find<ModBuff>("RiptideDebuff").Type, 120);
		}
		...
		public override bool PreDraw(ref Color lightColor) {
			lightColor = new Color(127, 255, 253, 0) * MathHelper.Lerp(1f, 0f, (float)Projectile.alpha / 255f);
