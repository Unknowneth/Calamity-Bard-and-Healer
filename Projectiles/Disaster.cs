...
{
	public class Disaster : ScythePro
	{
		private bool shouldSpin = false;
		public override string Texture => "CalamityBardHealer/Items/Disaster";
		...
		public override void SafeSetDefaults() {
			...
			base.Projectile.ArmorPenetration = 70;
			...
			if(base.Projectile.direction == -1) {
				if(base.Projectile.ai[1] / base.Projectile.ai[0] > 0.75f) {
					if(player.whoAmI == Main.myPlayer) {
						if(player.HeldItem.ModItem is Items.Disaster d) shouldSpin = d.spin >= 3;
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			Main.EntitySpriteDraw(texture, base.Projectile.Center - Main.screenPosition, null, lightColor, base.Projectile.rotation + MathHelper.PiOver2 - MathHelper.ToRadians(base.Projectile.spriteDirection * 15f), new Vector2(texture.Width / 2 - base.Projectile.spriteDirection * 21, texture.Height - 34), base.Projectile.scale, base.Projectile.spriteDirection < 0 ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0);
			if(base.Projectile.ai[1] / base.Projectile.ai[0] < 0.75f) {
				Player player = Main.player[base.Projectile.owner];
