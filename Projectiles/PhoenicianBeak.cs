...
{
	public class PhoenicianBeak : ScythePro
	{
		public override string Texture => "CalamityBardHealer/Items/PhoenicianBeak";
		...
		public override bool PreAI() {
			...
			if(base.Projectile.localAI[2] == 0f && base.Projectile.ai[1] <= base.Projectile.ai[0] * 0.5f) {
				if(Main.myPlayer == player.whoAmI) {
					int p = Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), base.Projectile.Center + Vector2.Normalize(base.Projectile.velocity) * base.Projectile.height * 0.6f, Vector2.Normalize(base.Projectile.velocity) * 24f, ModContent.ProjectileType<Projectiles.PhoenicianBeakSlice>(), base.Projectile.damage, base.Projectile.knockBack, player.whoAmI);
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			Main.EntitySpriteDraw(texture, base.Projectile.Center - Main.screenPosition, null, lightColor, base.Projectile.rotation + MathHelper.PiOver2 - MathHelper.ToRadians(base.Projectile.spriteDirection * 15f), new Vector2(texture.Width / 2 - base.Projectile.spriteDirection * 7, texture.Height - 44), base.Projectile.scale, base.Projectile.spriteDirection < 0 ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0);
			if(base.Projectile.ai[1] / base.Projectile.ai[0] < 0.75f) {
				Player player = Main.player[base.Projectile.owner];
				...
				Main.EntitySpriteDraw(texture, player.MountedCenter - new Vector2(4, 2) * player.Directions - Main.screenPosition, null, lightColor * swingTime * 0.9f, base.Projectile.rotation + MathHelper.ToRadians(base.Projectile.spriteDirection * 15f), texture.Size() / 2, base.Projectile.scale * 3.6f, base.Projectile.spriteDirection < 0 ? SpriteEffects.FlipVertically : SpriteEffects.None, 0);
				lightColor = Color.Gold;
				lightColor.A = 0;
				texture = (Texture2D)ModContent.Request<Texture2D>("CalamityBardHealer/Projectiles/Slash_1");
				Main.EntitySpriteDraw(texture, player.MountedCenter - new Vector2(4, 2) * player.Directions - Main.screenPosition, null, lightColor * swingTime * 0.9f, base.Projectile.rotation + MathHelper.ToRadians(base.Projectile.spriteDirection * 15f), texture.Size() / 2, base.Projectile.scale * 4.1f, base.Projectile.spriteDirection < 0 ? SpriteEffects.FlipVertically : SpriteEffects.None, 0);
