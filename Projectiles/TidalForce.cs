...
{
	public class TidalForce : ScythePro
	{
		public override string Texture => "CalamityBardHealer/Items/TidalForce";
		...
		public override bool PreAI() {
			...
			if(base.Projectile.localAI[2] == 0f && base.Projectile.ai[1] <= base.Projectile.ai[0] * 0.5f) {
				if(Main.myPlayer == player.whoAmI) {
					int p = Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), base.Projectile.Center + Vector2.Normalize(base.Projectile.velocity) * base.Projectile.height * 0.6f, Vector2.Normalize(base.Projectile.velocity) * 16f, ModContent.ProjectileType<Projectiles.TidalWave>(), base.Projectile.damage, base.Projectile.knockBack, player.whoAmI);
		...
		public override void SafeOnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			for(int i = 0; i < 7; i++) {
				...
				GeneralParticleHandler.SpawnParticle(new MediumMistParticle(target.Center + offset, velOffset * Main.rand.NextFloat(1.5f, 3f), Color.Aquamarine, Color.Blue, Main.rand.NextFloat(0.9f, 1.2f), 160f, Main.rand.NextFloat(0.03f, -0.03f)));
			}
