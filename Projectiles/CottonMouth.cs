...
{
	public class CottonMouth : ScythePro
	{
		public override string Texture => "CalamityBardHealer/Items/CottonMouth";
		...
		public override bool PreAI() {
			...
			if(base.Projectile.localAI[2] == 0f && base.Projectile.ai[1] <= base.Projectile.ai[0] * 0.5f) {
				if(Main.myPlayer == player.whoAmI) for(int i = -2; i <= 2; i++) {
					int p = Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), base.Projectile.Center + Vector2.Normalize(base.Projectile.velocity).RotatedBy(MathHelper.PiOver4 * 0.25f * i) * base.Projectile.height * 0.6f + Main.rand.NextVector2CircularEdge(i, i) * 2f, Vector2.Normalize(base.Projectile.velocity).RotatedBy(Main.rand.NextFloat(MathHelper.PiOver4 * 0.5f) - MathHelper.PiOver4 * 0.25f + MathHelper.PiOver4 * 0.25f * i) * Main.rand.NextFloat(6f, 9f), ModContent.ProjectileType<Projectiles.CottonMouthGas>(), base.Projectile.damage + base.Projectile.damage / 5, base.Projectile.knockBack, player.whoAmI);
		...
		public override void SafeOnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			for(int i = 0; i < 10; i++) {
				...
				GeneralParticleHandler.SpawnParticle(new MediumMistParticle(target.Center + offset, velOffset * Main.rand.NextFloat(1.5f, 3f), Color.Peru, Color.PeachPuff, Main.rand.NextFloat(0.9f, 1.2f), 160f, Main.rand.NextFloat(0.03f, -0.03f)));
				Dust dust = Dust.NewDustPerfect(target.Center + offset, Main.rand.NextBool(2) ? 288 : 207, new Vector2?(new Vector2(velOffset.X, velOffset.Y)), 0, default(Color), 0.6f);
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			Main.EntitySpriteDraw(texture, base.Projectile.Center - Main.screenPosition, null, lightColor, base.Projectile.rotation + MathHelper.PiOver2 - MathHelper.ToRadians(base.Projectile.spriteDirection * 15f), new Vector2(texture.Width / 2 - base.Projectile.spriteDirection * 13, texture.Height - 12), base.Projectile.scale, base.Projectile.spriteDirection < 0 ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0);
			if(base.Projectile.ai[1] / base.Projectile.ai[0] < 0.75f) {
				Player player = Main.player[base.Projectile.owner];
				float swingTime = base.Projectile.ai[1] / (base.Projectile.ai[0] * 0.75f);
				for(int e = 0; e < 4; e++) swingTime = MathHelper.SmoothStep(0f, 1f, swingTime);
				lightColor.A = 0;
				if(lightColor.R > Color.YellowGreen.R) lightColor.R = Color.YellowGreen.R;
				if(lightColor.G > Color.YellowGreen.G) lightColor.G = Color.YellowGreen.G;
				if(lightColor.B > Color.YellowGreen.B) lightColor.B = Color.YellowGreen.B;
				...
				Main.EntitySpriteDraw(texture, player.MountedCenter - new Vector2(4, 2) * player.Directions - Main.screenPosition, null, lightColor * swingTime * 0.8f, base.Projectile.rotation + MathHelper.ToRadians(base.Projectile.spriteDirection * 15f), texture.Size() / 2, base.Projectile.scale * 1.8f, base.Projectile.spriteDirection < 0 ? SpriteEffects.FlipVertically : SpriteEffects.None, 0);
			}
			return false;
		}
