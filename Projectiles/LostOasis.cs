...
{
	public class LostOasis : ModProjectile
	{
		...
		public override void AI() {
			...
			else foreach(Player target in Main.ActivePlayers) if(Projectile.Distance(target.MountedCenter) < 160f && !target.dead && Projectile.localAI[2] <= 0f && target.statLife < target.statLifeMax2) {
			...
			GeneralParticleHandler.SpawnParticle(new HeavySmokeParticle(Projectile.Center, Projectile.velocity * 0.5f, Color.Aquamarine, 20, Projectile.ai[0] / 12f, 0.6f, Main.rand.NextFloat(-MathHelper.PiOver4, MathHelper.PiOver4) * 0.2f, true, 0f, true));
		}
