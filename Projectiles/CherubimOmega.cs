...
{
	public class CherubimOmega : ScythePro
	{
		private bool shouldSpin = false;
		public override string Texture => "CalamityBardHealer/Items/CherubimOmega";
		...
		public override bool PreAI() {
			...
			if(base.Projectile.direction == -1) {
				if(base.Projectile.ai[1] / base.Projectile.ai[0] > 0.75f) {
					if(player.whoAmI == Main.myPlayer) {
						if(player.HeldItem.ModItem is Items.CherubimOmega co) shouldSpin = co.spin >= 3;
			...
			if(base.Projectile.localAI[2] == 0f && base.Projectile.ai[1] <= base.Projectile.ai[0] * 0.5f) {
				if(Main.myPlayer == player.whoAmI) if(!shouldSpin) {
					...
					int p = Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), Main.MouseWorld - shootDir * 480f, shootDir * 16f, ModContent.ProjectileType<Projectiles.CherubimSlash>(), base.Projectile.damage, base.Projectile.knockBack, player.whoAmI);
					NetMessage.SendData(27, -1, -1, null, p);
					p = Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), Main.MouseWorld - shootDir * 480f, shootDir * 32f, ModContent.ProjectileType<Projectiles.CherubimRift>(), base.Projectile.damage, base.Projectile.knockBack, player.whoAmI);
					NetMessage.SendData(27, -1, -1, null, p);
				}
				else for(int i = -1; i <= 1; i++) if(i != 0) {
					Vector2 shootDir = Vector2.Normalize(Main.MouseWorld - player.MountedCenter).RotatedBy(MathHelper.PiOver4 * i + Main.rand.Next(-10, 11) * MathHelper.PiOver4 * 0.02f);
					int p = Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), Main.MouseWorld - shootDir * 480f, shootDir * 16f, ModContent.ProjectileType<Projectiles.CherubimSlash>(), base.Projectile.damage, base.Projectile.knockBack, player.whoAmI);
					NetMessage.SendData(27, -1, -1, null, p);
					p = Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem), Main.MouseWorld - shootDir * 480f, shootDir * 32f, ModContent.ProjectileType<Projectiles.CherubimRift>(), base.Projectile.damage, base.Projectile.knockBack, player.whoAmI);
					NetMessage.SendData(27, -1, -1, null, p);
		...
		public override void SafeOnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			Color dustColor = Main.hslToRgb(Math.Abs(base.Projectile.rotation / MathHelper.TwoPi) % 1f, 1f, 0.66f);
			Color dustColor2 = Main.hslToRgb(Math.Abs((base.Projectile.rotation - MathHelper.Pi).ToRotationVector2().ToRotation() / MathHelper.TwoPi) % 1f, 1f, 0.66f);
			for(int i = 0; i < 10; i++) {
				...
				GeneralParticleHandler.SpawnParticle(new MediumMistParticle(target.Center + offset, velOffset * Main.rand.NextFloat(1.5f, 3f), dustColor, dustColor2, Main.rand.NextFloat(0.9f, 1.2f), 160f, Main.rand.NextFloat(0.03f, -0.03f)));
				Dust dust = Dust.NewDustPerfect(target.Center + offset, 278, new Vector2?(new Vector2(velOffset.X, velOffset.Y)), 0, Color.Lerp(dustColor, dustColor2, Main.rand.Next(11) * 0.1f), 0.6f);
				dust.noGravity = true;
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			if(base.Projectile.ai[1] / base.Projectile.ai[0] < 0.75f) {
				...
				lightColor = Main.hslToRgb(Math.Abs(rotation / MathHelper.TwoPi) % 1f, 1f, 0.66f);
				lightColor.A = 0;
				texture = (Texture2D)ModContent.Request<Texture2D>("CalamityMod/Particles/TrientCircularSmear");
				Main.EntitySpriteDraw(texture, player.MountedCenter - new Vector2(4, 2) * player.Directions - Main.screenPosition, null, lightColor * swingTime, base.Projectile.rotation, texture.Size() / 2, base.Projectile.scale * 1.8f, spriteEffects, 0);
