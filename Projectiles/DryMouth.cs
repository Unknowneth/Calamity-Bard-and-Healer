...
{
	public class DryMouth : ScythePro
	{
		public override string Texture => "CalamityBardHealer/Items/DryMouth";
		...
		public override void SafeOnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			for(int i = 0; i < 5; i++) {
				float rot = MathHelper.ToRadians(MathHelper.ToRadians(72f) * i);
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			Main.EntitySpriteDraw(texture, base.Projectile.Center - Main.screenPosition, null, lightColor, base.Projectile.rotation + MathHelper.PiOver2 - MathHelper.ToRadians(base.Projectile.spriteDirection * 15f), new Vector2(texture.Width / 2 - base.Projectile.spriteDirection * 10, texture.Height - 10), base.Projectile.scale, base.Projectile.spriteDirection < 0 ? SpriteEffects.FlipHorizontally : SpriteEffects.None, 0);
			if(base.Projectile.ai[1] / base.Projectile.ai[0] < 0.75f) {
				Player player = Main.player[base.Projectile.owner];
				float swingTime = base.Projectile.ai[1] / (base.Projectile.ai[0] * 0.75f);
				for(int e = 0; e < 4; e++) swingTime = MathHelper.SmoothStep(0f, 1f, swingTime);
				lightColor.A = 0;
				if(lightColor.R > Color.Peru.R) lightColor.R = Color.Peru.R;
				if(lightColor.G > Color.Peru.G) lightColor.G = Color.Peru.G;
				if(lightColor.B > Color.Peru.B) lightColor.B = Color.Peru.B;
